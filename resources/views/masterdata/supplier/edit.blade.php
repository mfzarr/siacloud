...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="supplierForm" action="{{ route('supplier.update', $supplier->id_supplier) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                                    ...
                                                    @foreach ($products as $product)
                                                        @if ($product->status == 'Aktif')
                                                            <option value="{{ $product->id_produk }}"
                                                                {{ in_array($product->id_produk, old('products', $supplier->products->pluck('id_produk')->toArray())) ? 'selected' : '' }}>
                                                                {{ $product->nama }}</option>
                                                        @endif
                                                    @endforeach
                                                </optgroup>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror"
                                        ...
                                        <option value="Tidak Aktif"
                                            {{ old('status', $supplier->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak
                                            Aktif</option>
                                    </select>
                                    @error('status')
                                        <span class="text-danger">{{ $message }}</span>
    ...
    <script>
        $('#multiple-select-field').select2({
            theme: "bootstrap-5",
            width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-50') ? '50%' : 'style',
        ...
        document.getElementById("saveButton").addEventListener("click", function() {
            ...
            Swal.fire({
                title: "Edit Supplier?",
                text: "Are you sure you want to edit this supplier?",
                icon: "question",
